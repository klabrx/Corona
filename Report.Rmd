...
if (file.exists(source.file.name)) Dashboard <- as.data.frame(read_excel(source.file.name)) %>%
  tail(100) %>%
  select(Meldedatum=Tag, Inzidenz=4)
...
### Zeitverlauf der Inzidenzen
```{r, InzPlot, eval=TRUE, echo=FALSE}
InzPlot <- timeline %>%
  ggplot(aes(x=Meldedatum, y=Inzidenz)) +
    geom_col() + 
